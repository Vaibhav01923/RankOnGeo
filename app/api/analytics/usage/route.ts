...
export async function GET(req: NextRequest) {
  ...
  const { data: userPlan } = await db.from("user_plans").select("plan, dodo_subscription_id, purchased_event_balance").eq("user_id", access.ownerId).maybeSingle();
  ...
  const { data: cycle } = await db.from("analytics_usage_cycles").select("period, ingestion_paused").eq("brand_id", brandId).maybeSingle();
